...
~~~~~~~~~~~~~~~~~~~~~
CLI frontend for the EVE PI Profit Engine.
...
Options:
    ...
    --tier INT           Minimum output tier to evaluate (1–4). Default: 3.
    --top INT            Show only the top N results by profit delta. Default: all.
    --verbose / -v       Show per-input material breakdown under each chain row.
    --poco-tax FLOAT     POCO export tax rate (0.0–1.0). Default: 0.05.
    --json               Emit results as JSON to stdout.
    --no-auth            Skip ESI auth and use mock prices (for offline testing).

Extracted from src/main.py in Phase 1 restructure.
"""
...
class _MockESIClient:
    ...
    def get_market_history(self, type_ids, region_id=None, days=5):
        """Return empty history — mock prices are all served as live orders."""
        return {}
...
def _print_table(results, verbose: bool = False) -> None:
    ...
    def _warn(price: float) -> str:
        """Return warning suffix if price is zero (no market data)."""
        return " ⚠" if price == 0.0 else ""
    ...
    print("=" * 148)
    print(
        f"  {'TIER':<6}{'PRODUCT':<38}{'SELL RAW (net)':<24}"
        f"{'PROCESS (net)':<24}{'DELTA':<22}"
        f"{'ISK/m³ (in)':<18}{'ISK/m³ (out)':<18}{'RECOMMENDATION'}"
    )
    print("=" * 148)
    for r in results:
        ...
        raw_tag  = " ⚠" if raw_missing else (" ~" if raw_history else "")
        proc_tag = " ⚠" if proc_missing else (" ~" if proc_history else "")
        raw_str  = f"{format_isk(r.sell_raw_net_isk)}{raw_tag}"
        proc_str = f"{format_isk(r.process_net_isk)}{proc_tag}"
        delta_str = f"+{format_isk(r.delta_isk)}" if r.delta_isk >= 0 else format_isk(r.delta_isk)
        in_den = format_isk(r.input_isk_per_m3)
        out_den = format_isk(r.output_isk_per_m3)
        ...
        if verbose:
            for inp in r.inputs:
                price_str = f"{inp.unit_price:,.0f} ISK"
                flag = "  ⚠ no market data" if inp.unit_price == 0.0 else ""
                print(f"       └ P{inp.tier}  {inp.name:<32} x{inp.qty:<8} {price_str}{flag}")
    print("=" * 148)
    if results:
        r0 = results[0]
        footer = (
            f"\n  Showing {len(results)} chain(s).  "
            f"Tax={r0.applied_sales_tax:.2%}  "
            f"BrokerFee={r0.applied_broker_fee:.2%}  "
            f"TransportRisk={r0.transport_risk_factor:.2%}  "
            f"POCOTax={r0.applied_poco_tax:.2%}"
        )
        print(footer)
    any_history = any(
        any(inp.price_source == "history" for inp in r.inputs) or r.output_price_source == "history"
        for r in results
    )
    if any_missing:
        print("  ⚠ = no active sell orders AND no market history — ISK value shown as 0")
    if any_history:
        print("  ~ = no active sell orders; price estimated from 5-day market history average")
    print()
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=[1, 2, 3, 4],
        default=3,
        help="Minimum output tier to evaluate (1–4, default: 3).",
    )
    parser.add_argument(
        "--top",
        type=int,
    ...
    parser.add_argument(
        ...
        dest="verbose",
        action="store_true",
        help="Show per-input material breakdown under each chain row.",
    )
    parser.add_argument(
        "--poco-tax",
        dest="poco_tax",
        ...
        default=0.05,
        metavar="RATE",
        help="POCO export tax rate (0.0–1.0). Default: 0.05 (5%% NPC POCO).",
    )
    parser.add_argument(
        "--json",
        dest="as_json",
