...
class ProfitEngine:
    ...
    def run(self, min_tier: int = 3) -> list[DecisionResult]:
        ...
        Args:
            min_tier: Minimum output tier to evaluate (3 or 4).
        ...
        for chain in chains:
            ...
            inputs = [
                MaterialPrice(
                    ...
                    tier=m["input_tier"],
                    qty=m["input_qty"],
                    unit_price=prices.get(m["input_type_id"], 0.0),
                    volume_m3=m["input_volume"],
                )
                for m in chain["materials"]
            ]

            output_price = prices.get(chain["output_type_id"], 0.0)
    ...
    def _fetch_prices(self, type_ids: list[int]) -> dict[int, float]:
        """Return best Jita sell price for each type ID."""
        import sys
        ...
        for type_id, order_list in orders.items():
            ...
            if sell_orders:
                prices[type_id] = min(o["price"] for o in sell_orders)
            else:
                prices[type_id] = 0.0
        return prices
